...
# FLAGS
DEFINE_string 'file'		''			''	'db'	'required'
DEFINE_string 'database'	''			''	'db'	'required'
DEFINE_string 'datadir'		'/var/lib/mysql'	''	'dr'
...
mysql -u'${FLAGS_user}' --execute 'drop database ${FLAGS_database}; create database ${FLAGS_database};' || exit 1
...
mysql ${FLAGS_database} < ${FLAGS_database}.ddl.sql
